...
{
    public class LiberarEMIsController : BaseController
    {
        public ActionResult Liberar()
        {
            LiberarEMIViewModel telaVM = CarregarViewBags(new LiberarEMIViewModel());
            return View(telaVM);
        }
        ...
        public ActionResult Liberar(LiberarEMIViewModel telaVM)
        {
        ...
        public ActionResult MontarSubEquipamento(int? id)
        {
            MontarSubEquipamentoViewModel telaVM = CarregarViewBags(new MontarSubEquipamentoViewModel());
            return View(telaVM);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        [MultipleButton(Name = "Montar", Argument = "Montar")]
        //public async Task<ActionResult> Editar([Bind(Include = "Imovel")] TelaManutencaoCorretivaViewModel telaVM)
        public ActionResult MontarSubEquipamento(MontarSubEquipamentoViewModel telaVM)
        {
            //HttpFileCollection uploadedFiles = System.Web.HttpContext.Current.Request.Files;
            //imovelVM.ImovelArquivos = new List<ImovelArquivo>();
        ...
        private LiberarEMIViewModel CarregarViewBags(LiberarEMIViewModel _telaVM)
        {
        ...
        private MontarSubEquipamentoViewModel CarregarViewBags(MontarSubEquipamentoViewModel _telaVM)
        {
            //var garagems = ((SimNaoEnum[])Enum.GetValues(typeof(SimNaoEnum))).Select(c => new { ID = (int)c, Name = c.ToString() }).ToList();
            //ViewBag.Garagems = new SelectList(garagems, "ID", "Name", imovelVM.Imovel.Garagem);
